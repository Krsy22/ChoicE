...
class Geometric_Chooser(nn.Module):
    def __init__(self, dim_str, num_rel, num_experts=3):
        super(Geometric_Chooser, self).__init__()
    ...
    def Euclidean(self, ent_embs, rel_embs, triplets):
        ...
        return rhs_scores
    def hyperbolic(self, ent_embs, rel_embs, triplets):
...
class ChoicE(nn.Module):
    def __init__(self, num_ent, num_rel, ent_vis, rel_vis, dim_vis, ent_txt, rel_txt, dim_txt, ent_vis_mask, rel_vis_mask, \
        ...
        self.ent_Multimodal_Chooser = CustomTransformerEncoder(ent_encoder_layer1, num_layer_enc_ent)
       
        rel_encoder_layer1 = CustomTransformerEncoderLayer(dim_str, num_head, 3, dim_hid, dropout)
        
        self.rel_Multimodal_Chooser = CustomTransformerEncoder(rel_encoder_layer1, num_layer_enc_rel)
        
        self.Geometric_Chooser = Geometric_Chooser(dim_str, num_rel, 3)
        self.init_weights()
    ...
    def forward(self):
        ...
        ent_embs = self.ent_Multimodal_Chooser(ent_seq, src_key_padding_mask = self.ent_mask)[:,0]
        ...
        rel_embs = self.rel_Multimodal_Chooser(rel_seq, src_key_padding_mask = self.rel_mask)[:,0]
    ...
    def score(self, emb_ent, emb_rel, triplets):
        ...
        scores, aux_loss = self.Geometric_Chooser(emb_ent, emb_rel, triplets)
